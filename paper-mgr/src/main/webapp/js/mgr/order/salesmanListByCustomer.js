            ...
            if(common.isNotNull(content)){
                for ( var i = 0; i < content.length; i++) {
                    ...
                    if(common.isNull(customerId)){
                        t+="<td><div class='button-link red delete'><a href='#' onclick='addCustomer("+content[i].id+")'>继续添加客户</a></div></td>";
                    }else{
                        t+="<td><div class='button-link red delete'><a href='#' onclick='updateCustomer("+customerId+","+content[i].id+")'>继续修改客户</a></div></td>";
                    }
...
function updateCustomer(customerId,salesId){
    window.location=basepath+"/jsp/order/customerEdit.jsp?id="+customerId+"&salesId="+salesId;
}
